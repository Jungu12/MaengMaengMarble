...
//     */
//    @GetMapping("/{userId}")
//    public ResponseEntity<?> myDetail(@PathVariable String userId) {
...
//    }
     ...
     */
    @GetMapping("/user")
    public ResponseEntity<?> myDetail(@AuthenticationPrincipal String userId) {
        // 제대로 user_id를 가져오느냐 확인하기 위한 로그
        logger.debug("users(), userId = {}", userId);
...
//    }
    ...
    public ResponseEntity<?> changeNickname(@AuthenticationPrincipal String userId, @RequestParam String newNickname) {
        logger.debug("NicknameChange(), userId = {}, newnickname = {}", userId, newNickname);
        userService.changeNickname(userId, newNickname);
